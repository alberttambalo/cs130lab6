...
void render(driver_state& state, render_type type)
{
    ...
    if(type == render_type::triangle){
        data_geometry* geometries = new data_geometry[3];

        for(int i = 0; i < state.num_vertices; i++){
            float* temp_data = state.vertex_data + (i * state.floats_per_vertex);
            int pos = i % 3;
            geometries[pos].data = temp_data;

            data_vertex temp_vertex;
            temp_vertex.data = temp_data;

            state.vertex_shader(temp_vertex, geometries[pos], state.uniform_data);
            if(pos == 2){
                rasterize_triangle(state, geometries[0], geometries[1], geometries[2]);
            }
        }
    }
}
    ...
    const data_geometry& v1, const data_geometry& v2)
{
    ...
    //find bounds of box that contains the triangle
    ...
    for (int i = min_x; i <= max_x; i++) {
        for (int j = min_y; j <= max_y; j++) {
            //compute barycentric weights for triangle about point p = i,j
            float alpha = triangle_area(i, j, x[1], y[1], x[2], y[2]) / ABC_area;
            float beta =  triangle_area(x[0], y[0], i, j, x[2], y[2]) / ABC_area;
            float gamma = triangle_area(x[0], y[0], x[1], y[1], i, j) / ABC_area;
